...
public class GenerateDataPoints {
	public static void main(String args[]) throws IOException, ClassNotFoundException{
		...
		ESAWikiBody esaWikiTitle = new ESAWikiBody(esaObject.invertedIndex, esaObject.documents);
		esaWikiTitle.initWikiBody();
		...
		for (int folderIndex = 0; folderIndex<folderNames.length;folderIndex++){
			...
			for (File doc : fileNames){
				...
				documentVector = esaWikiTitle.getVector(wordArray);
